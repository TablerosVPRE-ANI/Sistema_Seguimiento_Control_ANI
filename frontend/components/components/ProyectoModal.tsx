...
import { X, MapPin, Calendar, TrendingUp, AlertCircle, Star, FileText } from 'lucide-react';
import { Proyecto, ProyectoConPredial } from '@/types';
...
export default function ProyectoModal({ proyecto, onClose }: ProyectoModalProps) {
  ...
  const [showSeguimientoPredial, setShowSeguimientoPredial] = useState(false);

  if (!proyecto) return null;

  const enSeguimiento = estaEnSeguimiento(proyecto.id);
  const proyectoConPredial = proyecto as ProyectoConPredial;
  const tieneDatosPrediales = proyectoConPredial.tieneDatosPrediales;
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
        <div className="bg-slate-900 rounded-2xl border border-white/20 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="sticky bottom-0 bg-slate-900 border-t border-white/10 p-6 flex justify-between items-center">
            <div className="flex gap-3">
              {tieneDatosPrediales && proyectoConPredial.resumenPredial && (
                <button
                  onClick={() => setShowSeguimientoPredial(true)}
                  className="px-6 py-3 bg-orange-500 hover:bg-orange-600 rounded-xl font-semibold transition-colors flex items-center gap-2 text-white"
                >
      ...
      {showSeguimientoPredial && tieneDatosPrediales && proyectoConPredial.resumenPredial && (
        <SeguimientoPredial
          resumen={proyectoConPredial.resumenPredial}
          onClose={() => setShowSeguimientoPredial(false)}
        />
      )}
    </>
  );
